...
scraper for http://www.karaoketexty.cz, http://www.karaoketexty.sk, http://www.karaoke-lirics.net or http://www.karaoketeksty.pl
...
class LyricsFetcher:
    ...
    def __init__( self ):
        self.base_url = "http://www.karaoketexty.cz"
        self.searchUrl = "http://www.karaoketexty.cz/search?q=%term%"
    ...
    def search(self, artist, title):
        ...
        songPartMatch =  re.search("<h2>P.+?.*?>\s*(.*?)</ul>", searchResponse, re.DOTALL | re.MULTILINE | re.UNICODE);
        print(songPartMatch.group(1));
        
        if not songPartMatch:
            return None;                

        searchResult = re.findall("<a href=\"(.*?)\".*?>\s*(.*?)</", songPartMatch.group(1), re.MULTILINE | re.DOTALL);
